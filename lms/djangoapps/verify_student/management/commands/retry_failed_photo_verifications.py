...
from lms.djangoapps.verify_student.models import SoftwareSecurePhotoVerification, SSOReverificationConfig
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--args-from-database',
            action='store_true',
            help='Use arguments from the SSOReverificationConfig model instead of the command line.',
        )

    def get_args_from_database(self):
        """ Returns an options dictionary from the current SSOReverificationConfig model. """

        sso_config = SSOReverificationConfig.current()
        if not sso_config.enabled:
            raise CommandError('SSOReverification is disabled, but --args-from-database was requested.')

        # We don't need fancy shell-style whitespace/quote handling - none of our arguments are complicated
        argv = sso_config.arguments.split()

        parser = self.create_parser('manage.py', 'sso_reverify')
