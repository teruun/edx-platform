...
class AuthTestCase(ContentStoreTestCase):
    ...
    def test_signin_and_signup_buttons_index_page(self, allow_account_creation, assertion_method_name):
        ...
        with mock.patch.dict(settings.FEATURES, {"ALLOW_PUBLIC_ACCOUNT_CREATION": allow_account_creation}):
            ...
            assertion_method(
                response,
                '<a class="action action-signup" href="https://localhost:18000/register?next=http%3A%2F%2Ftestserver%2F">Sign Up</a>'  # pylint: enable=line-too-long
            )
            self.assertContains(
                response,
                '<a class="action action-signin" href="/signin_redirect_to_lms?next=http%3A%2F%2Ftestserver%2F">Sign In</a>'  # pylint: enable=line-too-long
            )
